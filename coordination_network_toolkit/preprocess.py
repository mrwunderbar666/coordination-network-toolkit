...
def preprocess_twitter_v2_likes_retweets(
    ...
    try:
        ...
        if edge_count:
            raise ValueError(
                "This preprocess function requires an empty database.")
        ...
        for page in data_pages:
            ...
            if call_type in {"liking_users", "retweeted_by"}:
                ...
                if hydrate_client and reference_user_id not in user_to_username:
                    profile = hydrate_client.user_lookup(
...
def preprocess_twitter_v2_like_retweet_files(db_path: str, input_filenames: List[str]):
    ...
    preprocess_twitter_v2_likes_retweets(
        db_path, iterate_all_pages(input_filenames))


def academictwitteR_preprocess(db_path: str, input_filenames: List[str]):
    """

    Load JSON files generated by the R Package academictwitteR
    ...
    for message_file in input_filenames:
        ...
        try:
            # Try v2 format
            _academictwitteR_load(db_path, tweets)
...
def _academictwitteR_load(db_path: str, tweets: Iterable[str]):
...
def academictwittr_load_users(db_path: str, users: Iterable[str], debug=False):
    for user_file in users:
        print(f"Begin preprocessing {user_file} into {db_path}")
        with open(user_file, "r", encoding="utf-8") as f:
            users = json.load(f)
        try:
            _academictwittr_insert_users(db_path, users)
        except Exception as e:
            if debug:
                print(user_file, type(e), e)
        print(f"Done preprocessing {user_file} into {db_path}")


def _academictwittr_insert_users(db_path: str, users):
    ...
    for user in users:
        ...
        with sqlite3.connect(db_path) as con:
            con.execute(
                f"UPDATE edge set username = '{username}' WHERE user_id = '{user_id}'")
